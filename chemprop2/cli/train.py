...
from chemprop2.cli.common import (
    add_common_args,
    find_models,
    process_common_args,
    validate_common_args,
)
from chemprop2.cli.conf import chemprop2_TRAIN_DIR, NOW
from chemprop2.cli.utils import (
    LookupAction,
    Subcommand,
...
from chemprop2.cli.utils.args import uppercase
...
from chemprop2.data.datasets import _MolGraphDatasetMixin
from chemprop2.models import MPNN, MulticomponentMPNN, save_model
from chemprop2.nn import AggregationRegistry, LossFunctionRegistry, MetricRegistry, PredictorRegistry
from chemprop2.nn.message_passing import (
    AtomMessagePassing,
    BondMessagePassing,
...
from chemprop2.utils import Factory
...
def add_train_args(parser: ArgumentParser) -> ArgumentParser:
    ...
    parser.add_argument(
        ...
        "--save-dir",
        type=Path,
        help="Directory where training outputs will be saved (defaults to ``CURRENT_DIRECTORY/chemprop2_training/STEM_OF_INPUT/TIME_STAMP``)",
    )
    parser.add_argument(
        "--remove-checkpoints",
        action="store_true",
    ...
    train_args.add_argument(
        ...
        help="Specify the evaluation metrics. If unspecified, chemprop2 will use the following metrics for given dataset types: regression -> ``rmse``, classification -> ``roc``, multiclass -> ``ce`` ('cross entropy'), spectral -> ``sid``. If multiple metrics are provided, the 0-th one will be used for early stopping and checkpointing.",
    )
    train_args.add_argument(
        "--tracking-metric",
        default="val_loss",
...
def process_train_args(args: Namespace) -> Namespace:
    if args.output_dir is None:
        args.output_dir = chemprop2_TRAIN_DIR / args.data_path.stem / NOW
