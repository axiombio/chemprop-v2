...
from chemprop2.cli.utils import LookupAction, Subcommand, build_data_from_files, make_dataset
from chemprop2.models.utils import load_model, load_output_columns
from chemprop2.nn.metrics import LossFunctionRegistry
from chemprop2.nn.predictors import EvidentialFFN, MulticlassClassificationFFN, MveFFN
from chemprop2.uncertainty import (
...
class PredictSubcommand(Subcommand):
    COMMAND = "predict"
    HELP = "use a pretrained chemprop2 model for prediction"
...
def add_predict_args(parser: ArgumentParser) -> ArgumentParser:
    ...
    parser.add_argument(
        ...
        "--preds-path",
        type=Path,
        help="Specify path to which predictions will be saved. If the file extension is .pkl, it will be saved as a pickle file. Otherwise, chemprop2 will save predictions as a CSV. If multiple models are used to make predictions, the average predictions will be saved in the file, and another file ending in '_individual' with the same file extension will save the predictions for each individual model, with the column names being the target names appended with the model index (e.g., '_model_<index>').",
    )
    parser.add_argument(
        "--drop-extra-columns",
        action="store_true",
