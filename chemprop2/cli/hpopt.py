...
from chemprop2.cli.common import add_common_args, process_common_args, validate_common_args
from chemprop2.cli.train import (
    TrainSubcommand,
    add_train_args,
    build_datasets,
    build_model,
...
from chemprop2.cli.utils.command import Subcommand
from chemprop2.data import build_dataloader
from chemprop2.nn import AggregationRegistry, MetricRegistry
from chemprop2.nn.transforms import UnscaleTransform
from chemprop2.nn.utils import Activation
...
def add_hpopt_args(parser: ArgumentParser) -> ArgumentParser:
    hpopt_args = parser.add_argument_group("chemprop2 hyperparameter optimization arguments")
...
def process_hpopt_args(args: Namespace) -> Namespace:
    if args.hpopt_save_dir is None:
        args.hpopt_save_dir = Path(f"chemprop2_hpopt/{args.data_path.stem}")
...
def tune_model(
    ...
    match args.raytune_search_algorithm:
        ...
        case "hyperopt":
            if NO_HYPEROPT:
                raise ImportError(
                    "HyperOptSearch requires hyperopt to be installed. Use 'pip install -U hyperopt' to install or use 'pip install -e .[hpopt]' in chemprop2 folder if you installed from source to install all hpopt relevant packages."
                )
        ...
        case "optuna":
            if NO_OPTUNA:
                raise ImportError(
                    "OptunaSearch requires optuna to be installed. Use 'pip install -U optuna' to install or use 'pip install -e .[hpopt]' in chemprop2 folder if you installed from source to install all hpopt relevant packages."
                )
...
def main(args: Namespace):
    if NO_RAY:
        raise ImportError(
            "Ray Tune requires ray to be installed. If you installed chemprop2 from PyPI, run 'pip install -U ray[tune]' to install ray. If you installed from source, use 'pip install -e .[hpopt]' in chemprop2 folder to install all hpopt relevant packages."
        )
