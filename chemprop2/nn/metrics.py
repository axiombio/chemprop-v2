...
from chemprop2.utils.registry import ClassRegistry
...
class chemprop2Metric(torchmetrics.Metric):
...
LossFunctionRegistry = ClassRegistry[chemprop2Metric]()
MetricRegistry = ClassRegistry[chemprop2Metric]()


@LossFunctionRegistry.register("mse")
@MetricRegistry.register("mse")
class MSE(chemprop2Metric):
...
class R2Score(torchmetrics.R2Score):
    def __init__(self, task_weights: ArrayLike = 1.0, **kwargs):
        ...
        task_weights :  ArrayLike = 1.0
            .. important::
                Ignored. Maintained for compatibility with :class:`chemprop2Metric`
        """
        super().__init__()
        task_weights = torch.as_tensor(task_weights, dtype=torch.float).view(1, -1)
        self.register_buffer("task_weights", task_weights)
...
class EvidentialLoss(chemprop2Metric):
...
class BinaryMCCLoss(chemprop2Metric):
    def __init__(self, task_weights: ArrayLike = 1.0):
        """
        Parameters
        ----------
...
class MulticlassMCCLoss(chemprop2Metric):
...
class DirichletLoss(chemprop2Metric):
...
class Wasserstein(chemprop2Metric):
...
class QuantileLoss(chemprop2Metric):
